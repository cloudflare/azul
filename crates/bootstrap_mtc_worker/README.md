...
worker and its configuration crate are excluded from the workspace and are no
longer maintained, runnable, or deployable from this repository.
...
Once a landmark interval elapses, the sequencer produces a landmark subtree and the CA can issue **signatureless MTC certificates** — DER-encoded X.509 structures whose `signatureValue` encodes a Merkle inclusion proof into the landmark subtree rather than a traditional signature.
