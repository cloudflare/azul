...
use thiserror::Error;
use tlog_tiles::{
    CheckpointSigner, Hash, HashReader, LogEntry, PendingLogEntry, Tile, TileIterator, TlogError,
    TlogTile, UnixTimestamp, HASH_SIZE,
};
...
const UNHASHED_TILE_KEY: u8 = u8::MAX - 1;
...
impl SequenceState {
    ...
    pub(crate) async fn load<L: LogEntry>(
        ...
        if c.size() > 0 {
            ...
            let data_tile = level0_tile.with_data_path(L::Pending::DATA_PATH);
            ...
            if let Some(path_elem) = L::Pending::UNHASHED_PATH {
...
async fn sequence_entries<L: LogEntry>(
    ...
    // If configured, load the current partial unhashed tile, if any.
    let mut unhashed_tile = Vec::new();
    if L::Pending::UNHASHED_PATH.is_some() {
        if let Some(t) = edge_tiles.get(&UNHASHED_TILE_KEY) {
            if t.tile.width() < TlogTile::FULL_WIDTH {
...
fn stage_data_tile<L: LogEntry>(
    ...
    let tile = TlogTile::from_index(tlog_tiles::stored_hash_index(0, n - 1))
        .with_data_path(L::Pending::DATA_PATH);
    edge_tiles.insert(
        DATA_TILE_KEY,
        TileWithBytes {
            tile,
    ...
    impl TestLog {
        ...
        fn check(&self, size: u64) -> anyhow::Result<u64> {
            ...
            let last_tile = TlogTile::from_index(tlog_tiles::stored_hash_count(c.size() - 1))
                .with_data_path(StaticCTPendingLogEntry::DATA_PATH);
            ...
            for n in 0..last_tile.level_index() {
                let tile = if n == last_tile.level_index() {
                    ...
                    TlogTile::new(
                        ...
                        n,
                        TlogTile::FULL_WIDTH,
                        Some(StaticCTPendingLogEntry::DATA_PATH),
                    )
                };
