...
//! 7. Bad consistency proof (right size, wrong hashes) → 422.
//! 8. `old > checkpoint.size` → 400.
//! 9. Malformed body (missing blank line) → 400.
//!
//! # Key management
//!
...
use tlog_tiles::{
    consistency_proof, record_hash, stored_hashes, tree_hash, Ed25519CheckpointSigner, Hash,
    HashReader, TlogError, TreeWithTimestamp, HASH_SIZE,
};
use tlog_witness::{
...
async fn tlog_witness_end_to_end() {
    ...
    // ----------------------- (8) old_size > checkpoint.size → 400 -----------------------
    {
