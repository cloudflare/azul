...
pub fn build_precert_tbs(tbs: &TbsCertificate) -> Result<Vec<u8>, StaticCTError> {
    ...
    let filtered_extensions: Vec<&Extension> = extensions
        .iter()
        .enumerate()
    ...
    // Re-encode TBS field-by-field (all fields are private in x509-cert 0.3).
...
mod tests {
    ...
    /// Uses field-by-field DER re-encoding since x509-cert 0.3 fields are private.
    fn rebuild_cert_with_extensions(
        ...
        if tbs.version() != x509_cert::certificate::Version::V1 {
            ...
            tagged.encode_to_vec(&mut tbs_content).unwrap();
        }
        tbs.serial_number().encode_to_vec(&mut tbs_content).unwrap();
        tbs.signature().encode_to_vec(&mut tbs_content).unwrap();
        tbs.issuer().encode_to_vec(&mut tbs_content).unwrap();
        tbs.validity().encode_to_vec(&mut tbs_content).unwrap();
        tbs.subject().encode_to_vec(&mut tbs_content).unwrap();
        tbs.subject_public_key_info()
            .encode_to_vec(&mut tbs_content)
            .unwrap();
        if let Some(uid) = tbs.issuer_unique_id() {
        ...
        der::Header::new(
            der::Tag::Sequence,
            der::Length::try_from(tbs_content.len()).unwrap(),
        )
        .encode_to_vec(&mut tbs_der)
        .unwrap();
        tbs_der.extend(&tbs_content);
