...
//!
//! The adapter holds an internal `Vec<u8>` byte buffer and a read
//! offset. Callers pull more bytes from the underlying stream
//! asynchronously (via [`StreamBuffer::pull_one`]), then parse
//! synchronously over a [`Cursor`](std::io::Cursor) wrapping the buffered
//! slice. The natural parse pattern is a retry loop: attempt the parse,
//! and on [`std::io::ErrorKind::UnexpectedEof`] pull another chunk and
//! retry. On success, [`StreamBuffer::consume`] advances the read offset
//! past the parsed bytes so subsequent parses start at the next byte.
//!
//! [`StreamBuffer::consume`] only bumps the read offset (O(1)); the dead
//! prefix is reclaimed lazily by the next [`StreamBuffer::pull_one`],
//! which shifts the live tail to the front before appending. This keeps
//! per-package consumes cheap and bounds the shift to at most once per
//! pull.
//!
...
pub(crate) struct StreamBuffer<S> {
    stream: S,
    buf: Vec<u8>,
    /// Read offset into `buf`: bytes before it have been consumed and
    ...
    S: Stream<Item = std::result::Result<Vec<u8>, E>> + Unpin,
{
    ...
    pub async fn pull_one(&mut self) -> std::result::Result<bool, E> {
        ...
        match self.stream.next().await {
            Some(Ok(chunk)) => {
                // Reclaim the consumed prefix before growing, so the
                // buffer tracks only live bytes plus one chunk.
                if self.start > 0 {
    ...
    /// View the currently-buffered (live) bytes. Used to construct a
    /// sync `Cursor` for parsing.
    ...
    /// Advance the read offset past the first `n` live bytes. The caller
    /// should call this after a successful parse to skip consumed input.
    /// The bytes are not freed here; the next [`Self::pull_one`] reclaims
    /// them.
    ///
    /// # Panics
...
mod tests {
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn pull_reclaims_consumed_prefix() {
        let mut buf = from_chunks(vec![b"abcd".to_vec(), b"efgh".to_vec()]);
        buf.pull_one().await.unwrap();
        buf.consume(3);
        assert_eq!(buf.buffered(), b"d");
        // The next pull shifts the live tail ("d") to the front, then
        // appends the new chunk.
        buf.pull_one().await.unwrap();
        assert_eq!(buf.buffered(), b"defgh");
        assert_eq!(buf.len(), 5);
    }
