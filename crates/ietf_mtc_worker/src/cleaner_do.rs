...
use ietf_mtc_api::{IetfMtcPendingLogEntry, LandmarkSequence, LANDMARK_KEY, SUBTREE_SIG_KEY_PREFIX};
...
fn parse_subtree_sig_hi(key: &str) -> Option<u64> {
    let suffix = key.strip_prefix(SUBTREE_SIG_KEY_PREFIX)?.strip_prefix('/')?;
