    ...
    pub fn subtree_for_index(&self, leaf_index: u64) -> Option<(usize, Subtree)> {
        ...
        match self.landmarks.binary_search(&leaf_index) {
            Ok(lo_index) => {
                ...
                hi = self.landmarks.get(lo_index + 1).copied();
                landmark_id = self.last_landmark - (self.landmarks.len() - 1) + lo_index;
            }
            Err(hi_index) => {
                // The leaf index doesn't exactly align with a landmark, but we
                // have the index of the next higher landmark.
                if hi_index > 0 {
                    lo = Some(self.landmarks[hi_index - 1]);
                } else {
                    lo = None;
                }
                hi = self.landmarks.get(hi_index).copied();
                landmark_id = self.last_landmark + 1 - self.landmarks.len() + hi_index - 1;
            }
        }
        match (lo, hi) {
            (Some(lo), Some(hi)) => {
...
mod tests {
    ...
    fn test_subtree_for_index() {
        ...
        // Check landmark sequence not at partial capacity.
        for i in 1..=5 {
            seq.add(i * 10).unwrap();
        }
        assert_eq!(seq.first_index(), 0);
        ...
        assert_eq!(
            ...
            Some((10, Subtree::new(100, 104).unwrap()))
        );
        // At last landmark.
        assert_eq!(
            seq.subtree_for_index(199),
            Some((19, Subtree::new(192, 200).unwrap()))
        );
        // Past last landmark.
