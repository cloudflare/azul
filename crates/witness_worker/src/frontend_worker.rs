...
async fn add_checkpoint(State(env): State<Env>, body: Bytes) -> ApiResult<impl IntoResponse> {
    ...
    //
    // `tlog_checkpoint::open_checkpoint` is deliberately not used here
    // because it additionally requires *every* configured verifier to sign
    // — the full-coverage semantics an issuer or monitor wants, but not a
...
/// itself.
///
/// The stateless approach intentionally does NOT re-verify the
/// original log signature on the reference checkpoint. That's safe
/// because this witness only ever produces a self-cosignature inside
/// `add-checkpoint`, which itself requires a valid log signature, so
/// any checkpoint carrying this witness's `subtree/v1` line is
/// guaranteed to have been log-signed at cosign time. Per the spec's
/// threat model, `/sign-subtree` therefore inherits the trust window
/// of `/add-checkpoint`: if the witness's signing key is ever
/// compromised, both endpoints become forgeable in lockstep.
///
/// Flow:
///
/// 1. Parse the request body (malformed → 400).
...
async fn sign_subtree(State(env): State<Env>, body: Bytes) -> ApiResult<impl IntoResponse> {
    ...
    if tlog_core::verify_subtree_consistency_proof(
        &consistency_proof,
        cp_text.size(),
        *cp_text.hash(),
        &subtree,
...
// Both `MAX_*_BODY_SIZE` constants below are the per-handler cap
// inside the worker. The Workers runtime imposes its own platform
// cap (100 MB on Free / Standard / Workers Paid; configurable on
// Enterprise) before any of our code runs, so peak buffered memory
// is bounded by the platform first; these constants tighten that
// to the worst-case well-formed request shape so a malicious or
// misconfigured client can't push the parser into base64-decoding
// tens of megabytes of garbage.
