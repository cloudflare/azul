...
//! # What's in scope
//!
//! - [`add_checkpoint`]: request and response parsers for the
//!   [`POST /add-checkpoint`][add] endpoint, in both directions (log → witness
//!   and witness → log).
...
pub use add_checkpoint::{
    parse_add_checkpoint_request, parse_add_checkpoint_response,
    serialize_add_checkpoint_request, serialize_add_checkpoint_response, AddCheckpointRequest,
    TlogWitnessError, CONTENT_TYPE_TLOG_SIZE, MAX_CONSISTENCY_PROOF_LINES,
};
